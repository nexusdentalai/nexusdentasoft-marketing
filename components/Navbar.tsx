    ...
    <nav className="fixed top-0 left-0 right-0 z-50 bg-white/80 backdrop-blur-xl border-b border-gray-100">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 flex items-center justify-between h-16">
        ...
        <div className="hidden md:flex items-center gap-6">
          ...
          {signupsOpen ? (
            <a href={ctaHref} className="px-4 py-2 bg-[#2e7df7] text-white text-sm font-semibold rounded-lg hover:bg-blue-600 shadow-sm">{ctaLabel}</a>
          ) : (
            <button type="button" onClick={openModal} className="px-4 py-2 bg-[#2e7df7] text-white text-sm font-semibold rounded-lg hover:bg-blue-600 shadow-sm">{ctaLabel}</button>
          )}
        </div>
      ...
      {menu && (
        <div className="md:hidden bg-white border-t border-gray-100 px-6 py-4 space-y-3">
          ...
          {signupsOpen ? (
            <a href={ctaHref} onClick={() => setMenu(false)} className="block px-4 py-2 bg-[#2e7df7] text-white text-sm font-semibold rounded-lg text-center">{ctaLabel}</a>
          ) : (
            <button type="button" onClick={openModal} className="block w-full px-4 py-2 bg-[#2e7df7] text-white text-sm font-semibold rounded-lg text-center">{ctaLabel}</button>
          )}
        </div>
      )}
